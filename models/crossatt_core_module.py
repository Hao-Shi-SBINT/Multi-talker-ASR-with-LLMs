import torch
import torch.nn as nn
import torch.nn.functional as F


class AcousticCrossAttnCore(nn.Module):
    def __init__(self, hidden_size: int, mem_dim: int, attn_dim: int = 512, dropout: float = 0.0):
        """
        hidden_size: LLaMA hidden size (e.g., 4096)
        mem_dim:     acoustic feature dim (e.g., encoder/sep dim)
        attn_dim:    small bottleneck dim for attention (<< hidden_size)
        """
        super().__init__()
        # 投到低维空间做 attention
        self.q_proj = nn.Linear(hidden_size, attn_dim)
        self.k_proj = nn.Linear(mem_dim, attn_dim)
        self.v_proj = nn.Linear(mem_dim, attn_dim)

        # 把 context 映射回 LLaMA hidden 维度
        self.out_proj = nn.Linear(attn_dim, hidden_size)

        self.dropout = nn.Dropout(dropout)
        self.ln_in = nn.LayerNorm(hidden_size)
        self.ln_out = nn.LayerNorm(hidden_size)

    def forward(
        self,
        hidden,
        mem,
        mem_sep=None,
        mem_mask=None,
        mem_conf=None,
        mem_ctc_mask=None,
        ctc_modules=None,
    ):
        """
        hidden: [B, L_q, H]   文本侧 hidden
        mem:    [B, T_m, D]   声学 serialized / fused 特征
        mem_mask: [B, T_m] bool, True 表示 padding，要在 attn 里屏蔽
        """
        if mem is None:
            return hidden

        B, L_q, H = hidden.size()
        _, T_m, D = mem.size()

        # 1) LN + 线性映射到低维
        h_norm = self.ln_in(hidden)        # [B, L_q, H]
        Q = self.q_proj(h_norm)            # [B, L_q, A]
        K = self.k_proj(mem)               # [B, T_m, A]
        V = self.v_proj(mem)               # [B, T_m, A]

        # 2) scaled dot-product attention
        attn_scores = torch.matmul(
            Q, K.transpose(1, 2)
        ) / (Q.size(-1) ** 0.5)            # [B, L_q, T_m]

        if mem_mask is not None:
            # mem_mask: True 表示 padding，要设成 -inf
            attn_scores = attn_scores.masked_fill(
                mem_mask.unsqueeze(1),  # [B, 1, T_m]
                float("-inf"),
            )

        # 如果以后想用 CTC confidence，可以在这里加：
        # if mem_conf is not None:
        #     eps = 1e-6
        #     log_conf = torch.log(mem_conf.clamp(min=eps))  # [B, T_m]
        #     attn_scores = attn_scores + log_conf.unsqueeze(1)

        attn_weights = F.softmax(attn_scores, dim=-1)  # [B, L_q, T_m]
        attn_weights = self.dropout(attn_weights)

        # 3) 上下文向量: [B, L_q, A]
        context = torch.matmul(attn_weights, V)

        # 4) 映射回 hidden_size + LN
        context_h = self.out_proj(context)         # [B, L_q, H]
        out = self.ln_out(hidden + context_h)      # 纯 adapter 输出（还没做 gate）

        # 返回 refined hidden，由外部 gate 做：
        # hidden_new = hidden + gate * (out - hidden)
        return out



class PerLayerAcousticAdapterWrapper(nn.Module):
    """
    包装一个共享的 AcousticCrossAttnCore，在前面加一个很小的 per-layer MLP（类似 LoRA）。
    所有 wrapper 共享同一个 core，但各自的 MLP 不同。
    """
    def __init__(
        self,
        shared_core: nn.Module,
        hidden_size: int,
        mlp_rank: int = 256,
        use_pre_mlp: bool = True,
        use_post_mlp: bool = False,
    ):
        super().__init__()
        self.shared_core = shared_core   # 引用同一个 AcousticCrossAttnCore

        self.use_pre_mlp = use_pre_mlp
        self.use_post_mlp = use_post_mlp

        if use_pre_mlp:
            self.pre_mlp = nn.Sequential(
                nn.Linear(hidden_size, mlp_rank),
                nn.SiLU(),
                nn.Linear(mlp_rank, hidden_size),
            )
        else:
            self.pre_mlp = None

        if use_post_mlp:
            self.post_mlp = nn.Sequential(
                nn.Linear(hidden_size, mlp_rank),
                nn.SiLU(),
                nn.Linear(mlp_rank, hidden_size),
            )
        else:
            self.post_mlp = None

    def forward(
        self,
        hidden,
        mem,
        mem_sep=None,
        mem_mask=None,
        mem_conf=None,
        mem_ctc_mask=None,
        ctc_modules=None,
    ):
        x = hidden

        # 1) per-layer 小 MLP 前处理（残差）
        if self.pre_mlp is not None:
            x = x + self.pre_mlp(x)   # [B, L_q, H]

        # 2) 调用共享 core
        x_refined = self.shared_core(
            x,
            mem,
            mem_sep=mem_sep,
            mem_mask=mem_mask,
            mem_conf=mem_conf,
            mem_ctc_mask=mem_ctc_mask,
            ctc_modules=ctc_modules,
        )

        # 3) per-layer 小 MLP 后处理（可选）
        if self.post_mlp is not None:
            x_refined = x_refined + self.post_mlp(x_refined)

        return x_refined

